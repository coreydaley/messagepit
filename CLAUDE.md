...
internal/
  ...
  mailtrap/        Mailtrap Email Sending API stub        (port 8100)
  sendgrid/        SendGrid v3 Mail Send API stub         (port 8101)
...
| 8100–8199 | Email API stubs     | Mailtrap=8100, SendGrid=8101         |
...
| `internal/<provider>/compat_test.go` | **Generated by `/provider-compat` — never edit by hand** |
...
| `server/server.go` | `Listen<Provider>()` — exact same pattern as `ListenMailtrap()` |
...
| `compat_test.go` | `/provider-compat` skill | spec-derived fidelity; overwritten on each run |
...
Fidelity is continuously verified by `/provider-compat`:

1. Reads the implementation to infer **claimed scope** — what the twin currently handles
2. Fetches the real provider's OpenAPI spec
3. Generates `compat_test.go` from the spec within that scope
4. Runs the tests and reports the fidelity score + any drift

When you extend a stub, re-run `/provider-compat <name>` to expand the test surface. When a real provider updates their API, re-run to surface drift.
